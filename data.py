...
train_wkt_v4 = pd.read_csv(os.path.join('/Users/sky/Downloads/train_wkt_v4.csv'))
grid_sizes = pd.read_csv(os.path.join('/Users/sky/Downloads/grid_sizes.csv'),
...
train_names = dat.all_train_names
test_ids = [dat.train_IDs_dict_r[name] for name in test_names]
train_ids = [dat.train_IDs_dict_r[name] for name in train_names]
...
def generate_train_ids(cl):
    ...
    :return:
    '''
    df = dat.collect_stats()
    df = df.pivot(index='ImageId', columns='Class', values='TotalArea')
    df = df.fillna(0)
    df = df[df[dat.CLASSES[cl + 1]] != 0]

    train_names = sorted(list(df.index.get_values()))

    return [dat.train_IDs_dict_r[name] for name in train_names]
    ...
    while True:
        ...
        if not rotation or rotation == 1:
            ...
            if verbose:
                ...
                # 'Rotation angle : {0}(degree), {1: 0.2f}(radian)'. \
                #     format(int(angle), radian)
