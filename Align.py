...
if __name__ == '__main__':
    ...
    for phase in ['train', 'test']:
        ...
        len_names = {'train': len(dat.all_train_names), 'test': len(dat.all_test_names)}[phase]

        for id in range(len_names):

            img = dat.ImageData(id, phase=phase)
            img_id = img.image_id
            ...
            img_m_register = dat.affine_transform(img_m_orig.astype(np.float32), warp_matrix_m, shape_out)
            img_a_register = dat.affine_transform(img_a_orig.astype(np.float32), warp_matrix_a, shape_out)
            ...
            compare_m_register = np.stack([img_m_register_rescale, empty_channel, img_3_rescale], axis=-1).astype(
                np.uint8)

            compare_a_orig = np.stack([img_a_orig_rescale, empty_channel, img_3_rescale], axis=-1).astype(np.uint8)
            compare_a_register = np.stack([img_a_register_rescale, empty_channel, img_3_rescale], axis=-1).astype(
                np.uint8)
            ...
            cv2.imwrite('./image_alignment/comparison/{}/3_rescale.png'.format(img.image_id),
                        img_3_rescale[200:900, 300:1200])
            cv2.imwrite('./image_alignment/comparison/{}/m_orig.png'.format(img.image_id),
                        compare_m_orig[200:900, 300:1200, :])
            cv2.imwrite('./image_alignment/comparison/{}/m_register.png'.format(img.image_id),
                        compare_m_register[200:900, 300:1200, :])
            cv2.imwrite('./image_alignment/comparison/{}/a_orig.png'.format(img.image_id),
                        compare_a_orig[200:900, 300:1200, :])
            cv2.imwrite('./image_alignment/comparison/{}/a_register.png'.format(img.image_id),
                        compare_a_register[200:900, 300:1200, :])
